...
- **Free**: Langsung menuju Dashboard tanpa checkout.
- **Personal / Pro AI**: 
  - Belum Login: `Pricing` ➔ `Klik Mulai` ➔ `Register` ➔ `Checkout` (tanpa input data dua kali).
  - Sudah Login: `Pricing` ➔ `Klik Mulai` ➔ `Checkout`.
...
- **Pilihan Billing**:
  ...
  - **Tahunan**: Personal (Rp 490.000/thn - Hemat Rp 98K), Pro AI (Rp 1.490.000/thn - Hemat Rp 298K).
- **Ringkasan Pesanan (Order Summary)**:
  - Rincian Paket, Periode Billing, Harga, Diskon Promo, Total Bayar.
  - Opsi Kode Promo / Kupon (`EXAMIGO10` dll).

## 3. Halaman Pembayaran (`/payment/:orderId`) & Gateway Pakasir
- Integrated dengan **Pakasir Payment Gateway** (`2sQMuMLg4dUBaxaYE1t1hqvCOvdiQyqI`).
- Metode Pembayaran: **QRIS** (GoPay, OVO, DANA, ShopeePay) & **Virtual Account** (BCA, BNI, BRI, Mandiri).
- Real-time Status Verification & Webhook (`POST /api/payments/pakasir-webhook`).

## 4. Subscription Management & Lifecycle
- Status Akun & Kuota otomatis aktif hanya via Webhook Server terverifikasi.
- Downgrade/Cancellation: Akses tetap aktif sampai periode billing berakhir.
- Expired Safety: Data bank soal/materi pengguna tetap disimpan aman (tidak dihapus).
